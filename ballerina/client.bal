...
public isolated client class Client {
    ...
    public isolated function init(ConnectionConfig config) returns error? {
        ...
        self.amazonHost = self.region != DEFAULT_REGION ? re `$SERVICE_NAME`.replace(AMAZON_AWS_HOST, SERVICE_NAME + "." + self.region) : AMAZON_AWS_HOST;
        string baseURL = HTTPS + self.amazonHost;

        self.authType = config.authType;
        if (self.authType == AWS_STATIC_AUTH) {
