...
# + sessionToken - Optional session token for IAM role authentication
...
isolated function getIAMCredentials() returns IAMCredentials|error {
    ...
    // Get IMDSv2 token
    http:Response tokenResponse = check metadataClient->put("", {}, {[X_AWS_EC2_METADATA_TOKEN_TTL_SECONDS]: "21600"});
    string token = check tokenResponse.getTextPayload();

    // Get role name
    http:Client roleClient = check new (METADATA_BASE_URL);
    http:Response roleResponse = check roleClient->get("",
        headers = {
        [X_AWS_EC2_METADATA_TOKEN]: token
    }
    );
    string roleName = check roleResponse.getTextPayload();

    // Get credentials
    http:Client credClient = check new (METADATA_BASE_URL + "/" + roleName);
    http:Response credResponse = check credClient->get("",
        headers = {
        [X_AWS_EC2_METADATA_TOKEN]: token
    }
    );

    // Parse JSON payload with full error handling
    json|error jsonResponse = credResponse.getJsonPayload();
    if jsonResponse is error {
        return jsonResponse;
    }

    // Detailed type conversion with error handling
    IAMCredentials|error credsResult = jsonResponse.cloneWithType(IAMCredentials);
    if credsResult is error {
        return credsResult;
    }
